...
export default function ScraperDashboard() {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      for (const agentId of Array.from(runningAgents)) {
        try {
          const res = await fetch(`${API_BASE_URL}/scraper-logs/${agentId}`);
          if (res.ok) {
            ...
            setRunningAgents((prev) => {
              ...
              if (data.running) updated.add(agentId);
              else updated.delete(agentId);
              return updated;
            });
          }
        } catch {
  ...
  const handleRunScraper = async (agentId: number) => {
    ...
    // Fetch a log snapshot after triggering
    try {
      const res = await fetch(`${API_BASE_URL}/scraper-logs/${agentId}`);
      if (res.ok) {
        const data = await res.json();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-slate-700 rounded-lg shadow-lg p-6 border border-slate-600">
            <div className="text-purple-400 text-sm font-mono break-all mb-2">{API_BASE_URL}</div>
        ...
        <div className="bg-slate-700 rounded-lg shadow-lg p-6 mb-8 border border-slate-600">
          ...
          >
            <Play size={20} />
            {loading || runningAgents.size > 0 ? `Running (${runningAgents.size} agents)...` : "Run All Agents"}
          </button>
        </div>
            ...
            >
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={async () => {
                    try {
                      await fetch(`${API_BASE_URL}/stop-scraper/${agentId}`, { method: "POST" });
                      toast.info(`Stop requested for agent ${agentId}`);
                      // immediate poll to reflect state sooner
                      const res = await fetch(`${API_BASE_URL}/scraper-logs/${agentId}`);
                      if (res.ok) {
                        const data = await res.json();
                        setLogsByAgent((prev) => ({ ...prev, [agentId]: data.logs || [] }));
        ...
        <div className="mt-8 bg-slate-700 rounded-lg shadow-lg p-6 border border-slate-600">
          ...
          <div className="space-y-2 text-slate-300 text-sm">
            ...
            <p className="text-yellow-300 mt-4">
              ⚠️ Each agent scraping job may take several minutes.
            </p>
          </div>
        </div>
      </div>
