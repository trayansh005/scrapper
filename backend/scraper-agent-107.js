...
// Helper function to format price in UK style
			...
			const properties = await page.$$eval(".tease-property", (elements) => {
				...
				elements.forEach((element) => {
					try {
						...
						// Extract price from .amount and sanitize
					...
					batch.map(async (property) => {
						...
						try {
							...
							if (existingRows.length > 0) {
								...
								console.log(
									`✅ Updated: ${property.link.substring(0, 50)}... | Price: ${formatPrice(
										property.price
								...
								console.log(
									`✅ Created: ${property.link.substring(0, 50)}... | Price: ${formatPrice(
										property.price
									)} | Coords: ${coords.latitude}, ${coords.longitude}`
								);
							} else {
							...
							if (coords.latitude && coords.longitude) {
								console.log(
									`✅ ${property.title} - ${formatPrice(property.price)} - ${coords.latitude}, ${
										coords.longitude
									}`
								);
							} else {
								console.log(`✅ ${property.title} - ${formatPrice(property.price)} - No coords`);
							}
						} catch (error) {
