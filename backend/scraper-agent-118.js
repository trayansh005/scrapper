// Mann & Co scraper using CheerioCrawler with JSON extraction
// Agent ID: 118
...
async function handleListingPage({ $, request }) {
    ...
    for (const prop of properties) {
        const fullUrl = prop.url.startsWith("http") ? prop.url : `https://www.manncountrywide.co.uk${prop.url}`;
...
async function scrapeMann() {
    logger.step("Starting Mann & Co (JSON Extraction) scraper...");
    ...
    for (const type of PROPERTY_TYPES) {
        ...
        for (let pg = Math.max(1, startPage); pg <= totalPages; pg++) {
            const url =
                pg === 1
                    ? `https://www.manncountrywide.co.uk/${type.urlPath}`
                    : `https://www.manncountrywide.co.uk/${type.urlPath}/page-${pg}`;
    ...
    logger.step(
        `Completed Mann & Co - Total scraped: ${counts.totalScraped}, Total saved: ${counts.totalSaved}, New sales: ${counts.savedSales}, New lettings: ${counts.savedRentals}`,
    );
