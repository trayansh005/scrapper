// Mann & Co scraper using Playwright with Crawlee
// Agent ID: 118
// Website: manncountrywide.co.uk
// Usage:
// node backend/scraper-agent-118.js [startPage]
		...
		const properties = await page.evaluate(() => {
			...
			return cards.map((card) => {
				...
				if (link && !link.startsWith("http")) {
					link = "https://www.manncountrywide.co.uk" + link;
				}

				const titleEl = card.querySelector(".card__text-content");
				const title = titleEl ? titleEl.textContent.trim() : "Mann & Co Property";

				const bedEl = card.querySelector(".card__text-title");
	...
	return new PlaywrightCrawler({
		...
		preNavigationHooks: [
			async ({ page }) => {
				...
				// User provided a UA in their snippet
...
async function scrapeMannCo() {
	const args = process.argv.slice(2);
	const startPage = args.length > 0 ? parseInt(args[0]) : 1;
	const scrapeStartTime = new Date();

	logger.step(`Starting Mann & Co Scraper (Agent ${AGENT_ID})...`);

	const browserWSEndpoint = getBrowserlessEndpoint();
	const crawler = createCrawler(browserWSEndpoint);

	const totalPages = 88; // From user snippet
	const requests = [];

	for (let p = Math.max(1, startPage); p <= totalPages; p++) {
		const url = `https://www.manncountrywide.co.uk/properties/sales/status-available/most-recent-first/page-${p}#/`;
		requests.push({
			url,
			userData: {
				pageNumber: p,
	...
	logger.step(
		`Finished Mann & Co - Total scraped: ${stats.totalScraped}, Total saved: ${stats.totalSaved}`,
	);
