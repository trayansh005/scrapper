// Entwistle Green scraper using Playwright to bypass Cloudflare and fetch `.ljson` API
// Agent ID: 219
// Website: entwistlegreen.co.uk
// Usage:
// node backend/scraper-agent-219.js [startPage]
	...
	try {
		...
		for (const prop of properties) {
			...
			const link = relativeLink.startsWith("http")
				? relativeLink
				: `https://www.entwistlegreen.co.uk${relativeLink}`;
...
async function scrapeEntwistleGreen() {
	logger.step(`Starting Entwistle Green API scraper (Agent ${AGENT_ID})...`);
	...
	for (const type of PROPERTY_TYPES) {
		...
		for (let pg = effectiveStartPage; pg <= totalPages; pg++) {
			allRequests.push({
				url: `https://www.entwistlegreen.co.uk/${type.urlPath}/page-${pg}/#/`,
				userData: {
					pageNum: pg,
					totalPages,
					isRental: type.isRental,
					label: `${type.label}`,
					apiUrl: `https://www.entwistlegreen.co.uk/${type.urlPath}.ljson?page=${pg}`,
				},
			});
		}
	}
	...
	logger.step(
		`Completed Entwistle Green - Total scraped: ${counts.totalScraped}, Total saved: ${counts.totalSaved}, New sales: ${counts.savedSales}, New rentals: ${counts.savedRentals}`,
	);
