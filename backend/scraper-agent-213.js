...
// Agent ID: 213
// Usage:
// node backend/scraper-agent-213.js
...
// Configuration for Ryder & Dutton
// 10 properties per page; sales 99 pages, rent 7 pages
const PROPERTY_TYPES = [
	// {
	// 	// Sales
	// 	urlBase: "https://ryderdutton.co.uk/properties-for-sale/All?excludeSstc=1",
	// 	totalPages: 99,
	// 	recordsPerPage: 10,
	// 	isRental: false,
	// 	label: "SALES",
	...
	{
		// Rentals
		urlBase: "https://ryderdutton.co.uk/properties-to-rent/All?excludeSstc=1",
		totalPages: 7,
		recordsPerPage: 10,
		isRental: true,
		label: "RENTALS",
...
async function scrapeRyderDutton() {
	console.log(`\n🚀 Starting Ryder & Dutton scraper (Agent ${AGENT_ID})...\n`);
					...
					batch.map(async (property) => {
						...
						if (!coords.latitude || !coords.longitude) {
							...
							try {
								...
								if (detailCoords) {
									// If values look swapped (latitude outside [-90,90]) or heuristics indicate swap
									let lat = detailCoords.lat;
									let lng = detailCoords.lng;
									// Extra heuristic for Ryder & Dutton-like data where lat and lng may be inverted
									if (
										Math.abs(lat) <= 10 &&
										lng >= 49 &&
										lng <= 61 &&
	...
	console.log(
		`\n✅ Completed Ryder & Dutton - Total scraped: ${totalScraped}, Total saved: ${totalSaved}`
	);
}
