// John D Wood scraper using Playwright to bypass Cloudflare and fetch `.ljson` API
// Agent ID: 39
// Usage:
// node backend/scraper-agent-39.js [startPage]
...
async function handleListingPage({ page, request }) {
	...
	try {
		...
		// Attempt to wait for the main React container or results container to ensure CF is bypassed
		await page
			.waitForSelector(".results-page, #homeflow-search-results", { timeout: 15000 })
			.catch(() => {});
		...
		for (const prop of properties) {
			...
			const link = relativeLink.startsWith("http")
				? relativeLink
				: `https://www.johndwood.co.uk${relativeLink}`;

			// Basic skipping
			if (isSoldProperty(prop.status || "")) continue;

			// Price extraction
			const priceText = prop.priceWithoutQualifier || prop.priceValue || prop.price || "";
...
async function scrapeJohnDWood() {
	logger.step(`Starting John D Wood API scraper (Agent ${AGENT_ID})...`);
	...
	const PROPERTY_TYPES = [
		{
			urlPath: "all-properties-for-sale/status-available/most-recent-first",
			totalRecords: 900, // Approximate, will iterate through pages
			recordsPerPage: 30,
			isRental: false,
			label: "SALES",
		},
		{
			urlPath: "all-properties-to-rent/status-available/most-recent-first",
			totalRecords: 400, // Approximate
			recordsPerPage: 30,
			isRental: true,
			label: "LETTINGS",
		},
	...
	for (const type of PROPERTY_TYPES) {
		...
		for (let pg = effectiveStartPage; pg <= totalPages; pg++) {
			allRequests.push({
				url:
					pg === 1
						? `https://www.johndwood.co.uk/${type.urlPath}#/`
						: `https://www.johndwood.co.uk/${type.urlPath}/page-${pg}/#/`,
				userData: {
					pageNum: pg,
					totalPages,
					...
					apiUrl:
						pg === 1
							? `https://www.johndwood.co.uk/${type.urlPath}.ljson`
							: `https://www.johndwood.co.uk/${type.urlPath}/page-${pg}.ljson`,
				},
			});
		}
	}
	...
	logger.step(
		`Completed John D Wood - Total scraped: ${counts.totalScraped}, Total saved: ${counts.totalSaved}, New sales: ${counts.savedSales}, New rentals: ${counts.savedRentals}`,
	);
