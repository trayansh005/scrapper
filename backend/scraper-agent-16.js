...
function extractCoordinatesFromHTML(html) {
    ...
    console.log(`   📍 Final results - Lat:`, latMatch ? latMatch[1] : 'null', 'Lng:', lngMatch ? lngMatch[1] : 'null');
                ...
                const properties = await page.$$eval('.property-card-wrapper', (cards) => {
                    ...
                    cards.forEach((card) => {
                        try {
                            ...
                            // Get the price from h3.property-price
                            const priceEl = card.querySelector('.property-price');
                            const priceText = priceEl ? priceEl.textContent.trim() : '';
                            // Extract just the number, removing £, commas, and text like "Offers over"
